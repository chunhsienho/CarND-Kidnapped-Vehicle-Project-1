...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  default_random_engine gen;
	normal_distribution<double> N_v_pred(0.0f, sigma_motion[0]);
	normal_distribution<double> N_yawrate_pred(0.0f, sigma_motion[1]);

  for (vector<Particle>::iterator it = particles.begin() ; it != particles.end(); ++it) {
    Particle p = *it;
    double noise_v = N_v_pred(gen) * delta_t;
    double noise_yawrate = N_yawrate_pred(gen) * delta_t;

    // velocity and yaw_rate with noise for this particle
    double _velocity = velocity + noise_v;
    double _yaw_rate = yaw_rate + noise_yawrate;

    double dx =  _velocity / _yaw_rate * (sin(p.theta + _yaw_rate * delta_t) - sin(p.theta));
    double dy =  _velocity / _yaw_rate * (cos(p.theta) - cos(p.theta + _yaw_rate * delta_t));
    p.x += dx;
    p.y += dy;
    p.theta += _yaw_rate * delta_t;
  }
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (vector<Particle>::iterator it = particles.begin() ; it != particles.end(); ++it) {
    ...
    map<int, LandmarkObs> pred_prediction_map;  // stores id prediction mapping

    for(int i=0; i<map_landmarks.landmark_list.size(); i++){
      // Tanslate landmark to map space to car(particle) space
      ...
      pred_prediction_map[l.id_i] = obs_pred;
    }
    ...
    for (vector<LandmarkObs>::iterator o_it = observations.begin() ; o_it != observations.end(); ++o_it) {
      LandmarkObs obs = *o_it;
      LandmarkObs pred_obs = pred_prediction_map[obs.id];
